...
- (void)setupViews
{
    ...
    [self.menuCollectionView addGestureRecognizer:recognizer];
...
- (void)handleSwipeFrom:(UISwipeGestureRecognizer *)recognizer {
    ...
    if(recognizer.direction == UISwipeGestureRecognizerDirectionDown) {
        ...
        if ([cell isKindOfClass:[TUIMediaCollectionCell class]]) {
            [self onCloseMedia:cell];
        }
...
- (void)setCurMessage:(V2TIMMessage *)curMessage allMessages:(NSArray<V2TIMMessage *> *)allMessages {
    ...
    for (V2TIMMessage *message in allMessages) {
        TUIMessageCellData *data = [TUIMessageMediaDataProvider getCellData:message];
        if (data) {
