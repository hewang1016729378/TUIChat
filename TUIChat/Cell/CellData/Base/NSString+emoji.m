...
- (NSString *)getLocalizableStringWithFaceContent
{
    ...
    NSString *regex_emoji = @"\\[[a-zA-Z0-9\\/\\u4e00-\\u9fa5-Za-z0-9_]+\\]"; //匹配表情
    NSError *error = nil;
...
{
    NSString *content = self;
    NSString *regex_emoji = @"\\[[a-z\\s*A-Z\\s*0-9!-@\\/\\u4e00-\\u9fa5-Za-z0-9_]+\\]"; //匹配表情
    NSError *error = nil;
    NSRegularExpression *re = [NSRegularExpression regularExpressionWithPattern:regex_emoji options:NSRegularExpressionCaseInsensitive error:&error];
    if (re) {
        ...
        for(TUIFaceGroup *group in [TUIConfig defaultConfig].faceGroups) {
            for (TUIFaceCellData *face in group.faces) {
                if ([face.name isEqualToString:subStr]) {
                    ...
                    textAttachment.bounds = CGRectMake(0, -(textFont.lineHeight-textFont.pointSize + 5)/2, textFont.pointSize + 5, textFont.pointSize + 5);
