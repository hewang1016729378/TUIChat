...
- (CGFloat)getCellDataHeightAtIndex:(NSUInteger)index Width:(CGFloat)width {
    ...
    if ([msg.reuseId isEqualToString:@"TImageMessageCell"]) {
        height = [msg heightOfWidth:width] + 10;
    } else if ([msg.reuseId isEqualToString:@"TTextMessageCell"]) {
        height = [msg heightOfWidth:width];
        NSInteger line = ceil(height)/[UIFont systemFontOfSize:16].lineHeight;
        if (line - 1 > 1) {
            height = [msg heightOfWidth:width] + 9;
        } else {
            height = [msg heightOfWidth:width];
        }
    } else {
